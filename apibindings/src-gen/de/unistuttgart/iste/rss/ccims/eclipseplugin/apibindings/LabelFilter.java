...
public class LabelFilter implements Serializable {
    ...
    private Input<List<String>> colour = Input.undefined();
    ...
    public Input<List<String>> getColourInput() {
        return colour;
    }

    public LabelFilter setColour(List<String> colour) {
        this.colour = Input.optional(colour);
        return this;
    }

    public LabelFilter setColourInput(Input<List<String>> colour) {
        if (colour == null) {
            throw new IllegalArgumentException("Input can not be null");
        }
        this.colour = colour;
    ...
    public void appendTo(StringBuilder _queryBuilder) {
        ...
        if (this.colour.isDefined()) {
            _queryBuilder.append(separator);
            separator = ",";
            _queryBuilder.append("colour:");
            if (colour.getValue() != null) {
                _queryBuilder.append('[');
                {
                    String listSeperator1 = "";
                    for (String item1 : colour.getValue()) {
                        _queryBuilder.append(listSeperator1);
                        listSeperator1 = ",";
                        Query.appendQuotedString(_queryBuilder, item1.toString());
                    }
